...
GtkWidget *about_diag_create()
{
	...
	label = banner_create("TNLPad - About",33);
...
"	,-1);
	...
	gtk_table_attach(GTK_TABLE(table),label,0,5,0,1,GTK_FILL,GTK_FILL,5,2);
	gtk_table_attach(GTK_TABLE(table),text_scroll,0,5,1,2,GTK_FILL|GTK_EXPAND,GTK_FILL|GTK_EXPAND,5,0);
