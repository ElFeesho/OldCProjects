...
Sprite::Sprite(const char *filename) : sheet(NULL), w(0), h(0), ltime(0), frames(0), cframe(0), rot(0.0), const_time(true), delays(NULL)
{
	fstream sprite_file;
...
{
	sheet->rotation(rot);
//	cout << "CFRAME: " << cframe << " SHEETW: " << sheet->W() << " Width: " << w << " Height: " << h << endl;
//	cout << "Drawing at: X: " << x << " Y: " << y << " TX: " << (cframe%(sheet->W()/w))*w << " TY: " << (cframe/(sheet->W()/w))*h << " W: " << w << " H: " << h << endl;
