...
void convolute(unsigned char *image)
{
	...
	int scale = 1;
	unsigned char *new_img = malloc(640*480*3);
	int i;
	int k;
	for(i = 0;i<640;i++)
	{
		for(k = 0; k<480;k++)
		{
			...
			else /* There are pixels to the left and right :D */
			{
				...
				{
					/* R */
					int offset_r = i*3+(k*640*3);

					int red_conv = 0;
					int gre_conv = 0;
					int blu_conv = 0;
					red_conv = (image[(i-1)*3+((k-1)*640*3)]*mask[0]);
					red_conv += (image[i*3+((k-1)*640*3)]*mask[1]);
					red_conv += (image[(i+1)*3+((k-1)*640*3)]*mask[2]);
					red_conv += (image[(i-1)*3+(k*640*3)]*mask[3]);
					red_conv += (image[i*3+(k*640*3)]*mask[4]);
					red_conv += (image[(i+1)*3+(k*640*3)]*mask[5]);
					red_conv += (image[(i-1)*3+((k+1)*640*3)]*mask[6]);
					red_conv += (image[i*3+((k+1)*640*3)]*mask[7]);
					red_conv += (image[(i+1)*3+((k+1)*640*3)]*mask[8]);
					/* G */
					gre_conv = (image[((i-1)*3+((k-1)*640*3))+1]*mask[0])+(image[(i*3+((k-1)*640*3))+1]*mask[1])+(image[((i+1)*3+((k-1)*640*3))+1]*mask[2])+(image[((i-1)*3+(k*640*3))+1]*mask[3])+(image[(i*3+(k*640*3))+1]*mask[4])+(image[((i+1)*3+(k*640*3))+1]*mask[5])+(image[((i-1)*3+((k+1)*640*3))+1]*mask[6])+(image[(i*3+((k+1)*640*3))+1]*mask[7])+(image[((i+1)*3+((k+1)*640*3))+1]*mask[8]);
					/* B */
					blu_conv = (image[((i-1)*3+((k-1)*640*3))+2]*mask[0])+(image[(i*3+((k-1)*640*3))+2]*mask[1])+(image[((i+1)*3+((k-1)*640*3))+2]*mask[2])+(image[((i-1)*3+(k*640*3))+2]*mask[3])+(image[(i*3+(k*640*3))+2]*mask[4])+(image[((i+1)*3+(k*640*3))+2]*mask[5])+(image[((i-1)*3+((k+1)*640*3))+2]*mask[6])+(image[(i*3+((k+1)*640*3))+2]*mask[7])+(image[((i+1)*3+((k+1)*640*3))+2]*mask[8]);
					red_conv/=scale;
					gre_conv/=scale;
					blu_conv/=scale;
	...
	memcpy(image, new_img, 640*480*3);
	free(new_img);
}
