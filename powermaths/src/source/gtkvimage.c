...
static gboolean gtk_vimage_expose(GtkWidget *widget, GdkEventExpose *event)
{
	...
	cairo_set_source_rgb(cr,42405.0/65536.0,0.0,8481.0/65536.0);
	cairo_fill(cr);
	cairo_save(cr);
	cairo_scale(cr, ((double)widget->allocation.width)/((double)vimage->width), ((double)widget->allocation.height)/((double)vimage->height));
